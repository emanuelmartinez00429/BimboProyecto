...
El contrato tipado de WPF vive en una única lista de `DefinicionPermiso` de `CapaUI/Core/Permisos/Permiso.cs`: cada valor `Permiso` tiene exactamente un `codigo_accion` y una etiqueta visible. `SesionPermisos.Tiene` resuelve el código en modo cerrado; si una definición local falta, registra el error y deniega sin interrumpir el login. `BimboProyecto.Tests/Rbac/ContratoRbacTests.cs` valida cobertura, unicidad, referencias de UI/RPC y, cuando se define `BIMBO_POSTGRES_CONNECTION_STRING`, compara contra `public.acciones` y la definición remota de `cambiar_estado_usuario_seguro`.

Las migraciones nuevas deben nombrarse `yyyyMMddHHmmss_nombre.sql`, en minúsculas. Las tres migraciones históricas `202608240001` a `202608240003` conservan su formato legado porque ya forman parte del historial y no se renombran. Borradores, `temp_*.sql`, salidas `out.sql`/`diff.patch` y variantes `*_supermin.sql` no se versionan.
...
Las RPC empresariales de usuario conservan `p_id_solicitud`, validan RBAC por `codigo_accion` y emiten `USUARIO_CREADO`, `USUARIO_MODIFICADO`, `USUARIO_DESACTIVADO` o `USUARIO_ACTIVADO` solo si hubo mutación. Activar utiliza `USUARIOS_MODIFICAR`; desactivar utiliza `USUARIOS_ELIMINAR`. No existe `USUARIOS_ACTIVAR`.
