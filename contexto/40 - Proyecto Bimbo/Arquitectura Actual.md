...
> [!success] Actualizado 2026-09-21 — Notificaciones homologadas y relaciones por nombre
> La bandeja completa usa los filtros compartidos `ComboFiltroBox` dentro de `PanelFiltrosFluido`. `NotificacionDetalleWindow` fue reemplazado por `NotificacionDetalleModal`, alojado en el overlay de `MainWindow`, con la anatomía de Bitácora y pinceles `EmpresaPrimary*` dinámicos. El RPC `listar_mis_notificaciones` enriquece la metadata de salida con el nombre completo del usuario o el nombre del rol, sin reescribir el historial; el ID queda reservado para navegación y nunca se presenta como «Registro relacionado». La migración `20260921141136_mostrar_nombre_registro_en_notificaciones.sql` está aplicada y verificada en Supabase. Ver [[Módulo Notificaciones]] y [[Sesión 2026-09-21 - Homologación visual y nombres relacionados en Notificaciones]].
...
> Era la única ventana que el escalado propio rompía: `470×285` con `ResizeMode="NoResize"`, y el `LayoutTransform` escala el contenido pero no el marco, así que a 1,1× el contenido no entraba. Ahora es un `UserControl` sobre el overlay de `RolesView` con `ModalLayout.LimitarAlOverlay`, como los otros 14 modales — hereda el transform y avisa por `Cerrado`/`Guardado` en vez de `DialogResult`. En ese momento quedaba `NotificacionDetalleWindow`; el 2026-09-21 también fue migrado a overlay, por lo que ya no quedan excepciones conocidas al patrón. Arné de instanciación en verde (470×285, idéntico a antes). Ver [[Sesión 2026-09-20 - Preferencias por usuario (Fase 1 del escalado)]] y [[Sesión 2026-09-21 - Homologación visual y nombres relacionados en Notificaciones]].
...
| [[Módulo Notificaciones]]                                | 🟢 Operativo (homologación visual y nombres 2026-09-21)                           | Campana con cinco recientes, bandeja paginada con filtros compartidos, modal sobre overlay, severidad visual, relaciones por nombre, lectura/archivo individual y masivo atómico, emisores idempotentes, RPC/RLS/Realtime y navegación autorizada                         |
...
- [[Sesión 2026-09-21 - Homologación visual y nombres relacionados en Notificaciones]] — filtros compartidos, modal sobre overlay y nombres legibles
