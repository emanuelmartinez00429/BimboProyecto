...
public partial class PresentacionesViewModel : RealtimeAwareViewModel
{
    ...
    public OrdenPresentacion Orden
    {
        get => _orden;
        set
        {
    ...
    public void SeleccionarSugerencia(PresentacionDto p)
    {
        ...
        _ = NavegarAPaginaDeRegistroAsync(p);
    }

    private async Task NavegarAPaginaDeRegistroAsync(PresentacionDto dto)
    {
        var r = await _repo.GetPaginaDeRegistroAsync(dto, PageSize, BuildFiltros());
    ...
    private void Nuevo()
    {
        if (SesionPermisos.Tiene(Permiso.ModificarConfiguracion)) SolicitarNuevo?.Invoke();
    }

    [RelayCommand(CanExecute = nameof(HaySeleccionado))]
    private void Editar()
    {
        if (Seleccionado is not null && SesionPermisos.Tiene(Permiso.ModificarConfiguracion)) SolicitarEditar?.Invoke(Seleccionado);
    }
    ...
    private async Task CargarPaginaSilenciosamenteAsync(bool actualizarFilas, bool esInsert)
    {
        try
        {
            ...
            // La heurística de INSERT asume que los registros nuevos caen en la
            // última página, lo cual solo vale con el orden por id ascendente.
            // Con orden alfabético una presentación nueva puede aparecer en
            // cualquier página, así que ahí se refresca siempre.
            bool ordenPorId = _orden == OrdenPresentacion.IdAsc;
            bool debeActualizarFilas = actualizarFilas
                && (!esInsert || !ordenPorId || _page == nuevoTotalPages);
