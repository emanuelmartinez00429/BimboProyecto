...
public partial class ContactosFabricantesViewModel : RealtimeAwareViewModel
{
    ...
    private async Task CargarPaginaAsync()
    {
        ...
        var task    = _fabRepo.GetPagedAsync(_page, PageSize, filtros, _cts.Token);
    ...
    private Task RefrescarSugerenciasAsync() => _buscador.EjecutarAsync(
        ...
        async (q, ct) =>
        {
            var r = await _fabRepo.BuscarSugerenciasAsync(q, new FabricanteFiltros { IdEstado = Activo }, ct);
    ...
    public async Task CargarContactosAsync(bool silencioso = false)
    {
        ...
        var r = await _contactoRepo.GetByFabricanteAsync(FabricanteSeleccionado.Id, _cts.Token);
        if (Disposed) return;
    ...
    {
        if (SesionPermisos.Tiene(Permiso.CrearFabricante)) SolicitarNuevoContacto?.Invoke();
    }

    [RelayCommand(CanExecute = nameof(HayContactoSeleccionado))]
    private void EditarContacto()
    {
        if (ContactoSeleccionado is not null && SesionPermisos.Tiene(Permiso.ModificarFabricante))
    ...
    {
        if (ContactoSeleccionado is null || !SesionPermisos.Tiene(Permiso.ModificarFabricante)) return;
