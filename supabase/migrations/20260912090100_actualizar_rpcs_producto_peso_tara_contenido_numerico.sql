-- crear_producto_seguro / actualizar_producto_seguro: p_id_tara (FK al catalogo) pasa a
-- p_peso_tara (numeric), p_contenido pasa de varchar a numeric. Se dropea primero porque
-- cambia la firma (nombre/tipo de parametro), no solo el cuerpo.
DROP FUNCTION IF EXISTS public.crear_producto_seguro(character varying, character varying, integer, integer, integer, numeric, integer, integer, character varying, integer, numeric, uuid);
DROP FUNCTION IF EXISTS public.actualizar_producto_seguro(integer, character varying, character varying, integer, integer, integer, numeric, integer, integer, character varying, integer, numeric, uuid);
...
BEGIN
    ...
    INSERT INTO public.productos (
        ...
        peso_teorico, peso_tara, id_categoria, contenido, id_pais, precio_por_kg
    ) VALUES (
...
BEGIN
    ...
    IF v_registro_actual.nombre_producto IS NOT DISTINCT FROM trim(p_nombre_producto) AND
       ...
       v_registro_actual.peso_tara IS NOT DISTINCT FROM p_peso_tara AND
       v_registro_actual.id_categoria IS NOT DISTINCT FROM p_id_categoria AND
       v_registro_actual.contenido IS NOT DISTINCT FROM p_contenido AND
